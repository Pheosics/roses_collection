...
for fname in os.listdir(dirs):
 if fnmatch.fnmatch(fname, 'Spells*.csv'):
...
for fname in files:
 ...
 t = fname.split('Spells')[1].split('csv')[0]

 ## Interaction File Open
 teout = open('interaction_spells'+t.lower()+'.txt','w')
 teout.write('interaction_spells'+t.lower()+'\n')
 teout.write('\n[OBJECT:INTERACTION]\n')
