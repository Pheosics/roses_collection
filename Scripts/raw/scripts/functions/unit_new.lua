...
function makeUnitTable(unit)                                                                    --*
 ...
 ---- Attributes (Physical, Mental, and Custom)
 unitTable.Attributes = {}
 for attribute,x in pairs(unit.body.physical_attrs) do
  unitTable.Attributes[attribute] = {}
  unitTable.Attributes[attribute].Base = tostring(x.value)
 end
 for attribute,x in pairs(unit.status.current_soul.mental_attrs) do
  unitTable.Attributes[attribute] = {}
  unitTable.Attributes[attribute].Base = tostring(x.value)
 end
 for _,attribute in pairs(roses.BaseTable.CustomAttributes._children) do
  unitTable.Attributes[attribute] = {}
  unitTable.Attributes[attribute].Base = '0'
 end
 for _,attribute in pairs(unitTable.Attributes._children) do
  unitTable.Attributes[attribute].Change = '0'
  unitTable.Attributes[attribute].Class = '0'
  unitTable.Attributes[attribute].Item = '0'
  unitTable.Attributes[attribute].StatusEffects = {}
 end
 
 ---- To many skills to put them all in the table, only add when needed
 unitTable.Skills = {}
 
 ---- To many traits to put them all in the table, only add when needed
 unitTable.Traits = {}
 
 ---- Resistances (Only custom)
 unitTable.Resistances = {}
 for _,resistance in pairs(roses.BaseTable.CustomResistances._children) do
  unitTable.Resistances[resistance] = {}
  unitTable.Resistances[resistance].Base = '0'
  unitTable.Resistances[resistance].Change = '0'
  unitTable.Resistances[resistance].Class = '0'
  unitTable.Resistances[resistance].Item = '0'
  unitTable.Resistances[resistance].StatusEffects = {}
 end 
 
 ---- Stats (Only custom)
 unitTable.Stats = {}
 for _,stat in pairs(roses.BaseTable.CustomStats._children) do
  unitTable.Stats[stat] = {}
  unitTable.Stats[stat].Base = '0'
...
function changeAttribute(unit,attribute,change,dur,track,syndrome)                              --*
 ...
 elseif df.mental_attribute_type[attribute] then -- Mental Attributes
  ...
  -- Nothing needs to be done for custom attributes right now
 end
