...
local makeGUI = reqscript("functions/gui").makeGUI
...
function journalFill(widget, selection, token)
	if widget.ViewID == "buildingView" then
...
function Building(widget, selection, token)
	...
	elseif widget.CellName == "BuildingList" then
		...
		local str = split(check.text[1].token, ":")
	...
	elseif widget.CellName == "BuildingDetails" then
		...
		widget:insert("Text", Info.Description)
...
local function main(...)
	...
	journal = makeGUI(mainViewDetails, subViewDetails)
